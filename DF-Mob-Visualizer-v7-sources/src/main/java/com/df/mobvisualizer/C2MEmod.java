...
public final class C2MEmod implements ClientModInitializer {
    ...
    /**
     * Adds a block-level wireframe and a light top-face tint to the natural
     * surface blocks in a marked chunk. The vertical scan is intentionally
     * limited so a large history cannot make the render thread scan entire
     * world columns every frame.
     */
    private void drawMarkedBlocks(MinecraftClient client, ChunkMark mark, Vec3d cameraPos,
        ...
        int minY = Math.max(client.world.getBottomY(),
                client.world.getTopY(Heightmap.Type.WORLD_SURFACE,
                        mark.chunkX() * 16, mark.chunkZ() * 16) - 20);
        int maxY = client.world.getTopY(Heightmap.Type.WORLD_SURFACE,
                mark.chunkX() * 16 + 15, mark.chunkZ() * 16 + 15);
        ...
        for (int x = mark.chunkX() * 16; x < mark.chunkX() * 16 + 16; x++) {
            for (int z = mark.chunkZ() * 16; z < mark.chunkZ() * 16 + 16; z++) {
                int columnTop = client.world.getTopY(Heightmap.Type.WORLD_SURFACE, x, z);
                int fromY = Math.max(minY, columnTop - 12);
                for (int y = fromY; y <= columnTop; y++) {
                    BlockState block = client.world.getBlockState(new net.minecraft.util.math.BlockPos(x, y, z));
                    if (!isMarkedSurfaceBlock(block)) continue;
                    double bx = x - cameraPos.x;
                    double by = y - cameraPos.y;
                    double bz = z - cameraPos.z;
                    drawCube(lines, bx, by, bz, r, g, b, borderAlpha);
                    if (fills != null) {
                        drawTopFill(fills, bx + 0.015, by + 1.002, bz + 0.015,
                                r, g, b, fillAlpha * 0.55f);
                    }
                }
            }
        }

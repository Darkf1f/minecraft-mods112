...
public final class MobSettingsScreenV2 extends Screen {
    ...
    private void buildHud(int left) {
        ...
        button(left, 140, 310, "Прозрачность поверхности: " + percent(config.chunkOpacity),
                b -> { config.chunkOpacity = nextOpacity(config.chunkOpacity); save(); init(); });
        button(left, 165, 310, "Усиление заливки: " + String.format(Locale.ROOT, "%.1fx", config.chunkFillStrength),
                b -> { config.chunkFillStrength = nextStrength(config.chunkFillStrength); save(); init(); });
        button(left, 190, 310, "Прозрачность границы: " + percent(config.chunkBorderOpacity),
                b -> { config.chunkBorderOpacity = nextOpacity(config.chunkBorderOpacity); save(); init(); });
        toggle(left, 215, "Показывать игроков", config.showPlayers, () -> config.showPlayers = !config.showPlayers);
        button(left, 245, 310, "Сбросить позицию HUD", b -> { config.hudX = 8; config.hudY = 8; save(); });
    }

...
public final class C2MEmod implements ClientModInitializer {
    ...
    /** Entity ids whose glow was applied by this mod on the previous scan. */
    private final Set<Integer> ownedGlowIds = new HashSet<>();
    ...
    public void onInitializeClient() {
        ...
        WorldRenderEvents.AFTER_ENTITIES.register(this::renderThroughWalls);
    }
    ...
    private void tick(MinecraftClient client) {
        ...
        for (Entity entity : client.world.getEntities()) {
            if (ownedGlowIds.remove(entity.getId())) entity.setGlowing(false);
        }
        ownedGlowIds.clear();
        for (Entity entity : client.world.getEntities()) {
            ...
            TrackedMob tracked = new TrackedMob(id, entity.getType().toString(), entity.getName().getString(),
                    entity.getBlockX(), entity.getBlockY(), entity.getBlockZ(), alert, color,
                    player, hostile, hurt, chargedCreeper, entity.hasCustomName());
            if (mobHighlightsOpen && config.seeThroughMobs && shouldHighlight(tracked)) {
    ...
    private void renderThroughWalls(WorldRenderContext context) {
        if (!mobHighlightsOpen || !config.seeThroughMobs || MinecraftClient.getInstance().world == null
        ...
        try {
            for (Entity entity : client.world.getEntities()) {
                if (entity == client.player) continue;
                int id = entity.getId();
                TrackedMob mob = new TrackedMob(id, entity.getType().toString(), entity.getName().getString(),
                        entity.getBlockX(), entity.getBlockY(), entity.getBlockZ(), isAlert(entity, id, state.currentMaxId()),
                        MobColors.forEntity(entity.getType().toString(), id, state.currentMaxId(), config),
                        entity.isPlayer(), isHostile(entity.getType().toString()),
                ...
                dispatcher.render(entity, entity.getX() - cameraPos.x, entity.getY() - cameraPos.y,
                        entity.getZ() - cameraPos.z, entity.getYaw(), 0.0f, context.matrixStack(),
    ...
    private boolean isAlert(Entity entity, int id, int maxId) {
        if (!config.alertEnabled || entity.isPlayer() || entity.getType().toString().endsWith("enderman")
                || !matchesConfiguredType(entity.getType().toString(), config.alertEntityTypes)) return false;

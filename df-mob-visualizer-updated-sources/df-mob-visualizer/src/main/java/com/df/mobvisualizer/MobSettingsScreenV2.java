...
/**
 * Category-based settings UI. Lists are paged instead of being placed below
 * the screen, so every option remains reachable on small displays.
 */
public final class MobSettingsScreenV2 extends Screen {
    ...
    protected void init() {
        ...
        if (page.equals("main")) {
            ...
            button(left, 130, 310, "Подсветка через блоки  → галочки", b -> open("highlight"));
            button(left, 155, 310, "Цвета каждого моба", b -> open("colors"));
            button(left, 180, 310, "Клавиши / бинды", b -> open("keys"));
            button(left, 205, 310, "Цвета по ID и проценту", b -> open("idcolors"));
            button(left, 230, 310, "ALERT", b -> open("alert"));
            button(left, 255, 310, "Очистка данных", b -> open("cleanup"));
            button(left, 290, 310, "Готово", b -> close());
            return;
        }
        title(pageTitle());
        button(left, 35, 90, "← Разделы", b -> open("main"));
        if (page.equals("session") || page.equals("highlight") || page.equals("colors")) {
    ...
    private void buildEntityPage(int left) {
        ...
        int end = Math.min(entityIds.size(), start + 12);
        Set<String> selected = page.equals("session") ? parseSet(config.pinnedEntityTypes)
                : parseSet(config.highlightEntityTypes);
        ...
        for (int i = start; i < end; i++) {
            ...
            String suffix = page.equals("colors") ? "  " + hex(colors.getOrDefault(id, defaultColor(id)))
                    : "  [" + (on ? "✓" : " ") + "]";
            button(left, y, 310, name + suffix, b -> {
                if (page.equals("colors")) {
                    ...
                    else config.highlightEntityTypes = serializeSet(selected);
                }
                save();
                init();
            });
        }
        button(left - 80, 65 + 12 * 22, 70, "←", b -> {
            listPage = Math.max(0, listPage - 1); init();
        });
        button(left + 240, 65 + 12 * 22, 70, "→", b -> {
            if ((listPage + 1) * 12 < entityIds.size()) listPage++; init();
        });
        drawPageInfo = true;
    }
    ...
    private void buildKeys(int left) {
        button(left, 65, 150, keyLabel("HUD", config.hudKey, 1), b -> waitKey(1));
        button(left + 160, 65, 150, keyLabel("Мобы", config.mobHighlightsKey, 2), b -> waitKey(2));
        button(left, 90, 150, keyLabel("Настройки", config.settingsKey, 3), b -> waitKey(3));
        button(left + 160, 90, 150, keyLabel("Чанки", config.chunksKey, 4), b -> waitKey(4));
        button(left, 115, 150, keyLabel("Очистить сессию", config.clearSessionKey, 5), b -> waitKey(5));
        button(left + 160, 115, 150, keyLabel("Очистить чанки", config.clearChunksKey, 6), b -> waitKey(6));
        button(left, 150, 310, "Сбросить все клавиши", b -> {
            ...
            config.clearSessionKey = GLFW.GLFW_KEY_UNKNOWN; config.clearChunksKey = GLFW.GLFW_KEY_UNKNOWN;
    ...
    private void buildGeneral(int left) {
        ...
        toggle(left, 140, "Только hostile", config.hostileOnly, () -> config.hostileOnly = !config.hostileOnly);
        toggle(left, 165, "Другие сущности", config.includeOtherEntities, () -> config.includeOtherEntities = !config.includeOtherEntities);
        toggle(left, 190, "Мобы после удара", config.pinHurtMobs, () -> config.pinHurtMobs = !config.pinHurtMobs);
        toggle(left, 215, "Сохранять сессию", config.persistSession, () -> config.persistSession = !config.persistSession);
    }

    private void buildIdColors(int left) {
        idRulesField = new TextFieldWidget(textRenderer, left, 65, 310, 20,
                Text.literal("Гибкие правила цветов ID"));
        idRulesField.setMaxLength(4000);
        idRulesField.setText(config.idColorRules == null ? "" : config.idColorRules);
        idRulesField.setPlaceholder(Text.literal("id<5000=#C855E8FF;percent<42=#FFFFB000"));
        addDrawableChild(idRulesField);
        button(left, 95, 310, "Сохранить правила цветов ID", b -> {
            config.idColorRules = idRulesField.getText();
            save();
        });
    }
    ...
    private void buildHud(int left) {
        ...
        toggle(left, 115, "Показывать заливку чанков", config.showChunkFill, () -> config.showChunkFill = !config.showChunkFill);
        toggle(left, 140, "Показывать игроков", config.showPlayers, () -> config.showPlayers = !config.showPlayers);
        button(left, 170, 310, "Сбросить позицию HUD", b -> { config.hudX = 8; config.hudY = 8; save(); });
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        switch (waitingForKey) {
            case 1 -> config.hudKey = keyCode;
            case 2 -> config.mobHighlightsKey = keyCode;
            case 3 -> config.settingsKey = keyCode;
            case 4 -> config.chunksKey = keyCode;
            case 5 -> config.clearSessionKey = keyCode;
            case 6 -> config.clearChunksKey = keyCode;
        }
        waitingForKey = 0;
    ...
    private String keyLabel(String name, int key, int target) {
        return (waitingForKey == target ? "Нажми: " : name + ": ") + keyName(key);
    }

    private String keyName(int key) {
        return key == GLFW.GLFW_KEY_UNKNOWN ? "не назначено" : InputUtil.fromKeyCode(key, 0).getLocalizedText().getString();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (drawPageInfo && !page.equals("main")) {
            context.drawCenteredTextWithShadow(textRenderer,
                    Text.literal("Страница " + (listPage + 1) + " / " + Math.max(1, (entityIds.size() + 11) / 12)),
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontal, double vertical) {
        if (page.equals("session") || page.equals("highlight") || page.equals("colors")) {
            if (vertical < 0 && (listPage + 1) * 12 < entityIds.size()) listPage++;
    ...
    private String serializeColors(Map<String, Integer> values) {
        return values.entrySet().stream().sorted(Map.Entry.comparingByKey())
                .map(e -> e.getKey() + "=#" + String.format("%08X", e.getValue())).reduce((a, b) -> a + "," + b).orElse("");
    }

    private int defaultColor(String id) { return 0xFF55AAFF; }
    private String hex(int color) { return String.format("#%08X", color); }
    private void save() {

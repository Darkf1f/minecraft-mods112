    ...
    private void drawMarkedBlocks(MinecraftClient client, ChunkMark mark, Vec3d cameraPos,
        ...
        for (int x = mark.chunkX() * 16; x < mark.chunkX() * 16 + 16; x++) {
            for (int z = mark.chunkZ() * 16; z < mark.chunkZ() * 16 + 16; z++) {
                ...
                // A top-only outline follows the visible surface and keeps
                // neighboring blocks readable instead of drawing a wall.
                // Stay almost exactly on the block edges. The tiny inset and
                // vertical lift prevent z-fighting with the block texture.
                drawTopOutline(lines, bx + 0.001, by + 1.001, bz + 0.001,
                        r, g, b, borderAlpha);
                if (fills != null) {
                    drawTopFill(fills, bx + 0.001, by + 1.003, bz + 0.001,

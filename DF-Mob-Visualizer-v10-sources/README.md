...
- v10 removes the confusing chunk-wide surface quad from the renderer and
  draws one top-only outline for the actual surface block in every column of
  a marked chunk. Water, grass, dirt, leaves and other supported surface
  blocks are outlined individually, so the terrain remains readable.

...
public final class C2MEmod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // Draw after terrain/translucent world content, while Fabric's shared
        // world consumers are still open. LAST is too late: its consumers
        // have already been closed and writing to them throws Not building.
     ...
     * standard world line layer without a manually managed vertex buffer.
     * This callback is registered before the final world-buffer flush.
     */
    ...
    private void renderChunks(WorldRenderContext context) {
        ...
        try {
            ...
            VertexConsumer blockFills = null;
            if (config.showChunkFill) {
                try {
                    // Keep the fill layer isolated: if a debug quad layer is
                    // unavailable in a particular client build, the reliable
                    // line overlay must still be rendered.
                    RenderSystem.setShader(ShaderProgramKeys.POSITION_COLOR);
                    blockFills = context.consumers().getBuffer(RenderLayer.getDebugQuads());
                } catch (IllegalStateException error) {
                    System.err.println("[DF Mob Visualizer] Surface fill layer unavailable: "
                            + error.getMessage());
                }
            }
            RenderSystem.lineWidth(2.0f);

            // A visible probe makes the render path testable even before a
            // matching mob has created the first persistent ChunkMark.
            if (surfaceProbe && MinecraftClient.getInstance().player != null) {
                Entity player = MinecraftClient.getInstance().player;
                drawMarkedBlocks(client,
                        new ChunkMark(player.getChunkPos().x, player.getChunkPos().z,
                                0xFFFF00FF, true, false, 0L),
                        cameraPos, vertices, blockFills,
                        1.0f, 0.0f, 1.0f);
            }
            ...
            // but do not hide the reason while diagnosing the overlay.
            System.err.println("[DF Mob Visualizer] Chunk overlay render failed: "
                    + error.getMessage());
        } finally {
            RenderSystem.depthMask(true);
            RenderSystem.enableDepthTest();
    ...
    private void drawWaypointCorners(VertexConsumer lines, ChunkMark mark, Vec3d cameraPos,
    ...
    private static void linePlain(VertexConsumer v, double x1, double y1, double z1,
                                  ...
                                  float r, float g, float b, float a) {
        // RenderLayer.getLines() in Minecraft 1.21.4 requires a normal
        // element even for debug lines. Omitting it aborts the whole LAST
        // world-render callback before the chunk surface can be drawn.

...
public final class C2MEmod implements ClientModInitializer {
    ...
    public static void applyKeyConfig(MobOverlayConfig config) {
        TOGGLE_HUD.setBoundKey(InputUtil.fromKeyCode(config.hudKey, config.hudScanCode));
        TOGGLE_CHUNKS.setBoundKey(InputUtil.fromKeyCode(config.chunksKey, config.chunksScanCode));
        TOGGLE_MOB_HIGHLIGHTS.setBoundKey(InputUtil.fromKeyCode(config.mobHighlightsKey, config.mobHighlightsScanCode));
        OPEN_SETTINGS.setBoundKey(InputUtil.fromKeyCode(config.settingsKey, config.settingsScanCode));
        CLEAR_SESSION.setBoundKey(InputUtil.fromKeyCode(config.clearSessionKey, config.clearSessionScanCode));
        CLEAR_CHUNKS.setBoundKey(InputUtil.fromKeyCode(config.clearChunksKey, config.clearChunksScanCode));
    }
    ...
    private static boolean scanFallbackDown(MinecraftClient client, int keyCode, int scanCode) {
        return (keyCode == GLFW.GLFW_KEY_UNKNOWN || keyCode == 0) && scanCode > 0
                && InputUtil.isKeyPressed(client.getWindow().getHandle(), scanCode);
    }

...
public final class MobSettingsScreenV2 extends Screen {
    ...
    protected void init() {
        ...
        if (page.equals("main")) {
            ...
            button(left, 230, 310, "Очистка данных", b -> open("cleanup"));
            button(left, 265, 310, "Готово", b -> close());
            return;
        }
        title(pageTitle());
    ...
    private void buildGeneral(int left) {
        ...
        toggle(left, 140, "Мобы после удара", config.highlightHurtMobs, () -> config.highlightHurtMobs = !config.highlightHurtMobs);
    ...
    private void buildHud(int left) {
        ...
        toggle(left, 115, "Только враждебные", config.hostileOnly, () -> config.hostileOnly = !config.hostileOnly);
    ...
    private void save() { config.normalize(); config.save(); }

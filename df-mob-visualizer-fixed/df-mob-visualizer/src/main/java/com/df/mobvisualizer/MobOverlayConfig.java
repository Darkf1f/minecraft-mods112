...
public final class MobOverlayConfig {
    /** Bumps only when defaults need to be safely migrated for existing users. */
    ...
    public String idColorRules = "id<=10000=#C855E8FF";
    ...
    public boolean showChunkOverlay = true;
    ...
    public void normalize() {
        // Older builds defaulted to a disabled map, which made a successful
        // scan look like a broken mod after an upgrade. Migrate that config
        // once, while preserving any later user toggle.
        if (configVersion < 1) {
            enabled = true;
            showChunkOverlay = true;
            showChunkFill = true;
            if ("id<=10001=#C855E8FF".equalsIgnoreCase(idColorRules)) {
                idColorRules = "id<=10000=#C855E8FF";
            }
            configVersion = 1;
        }
        ...
        if (idColorRules == null || idColorRules.isBlank()) {
            idColorRules = "id<=10000=#C855E8FF";
        }
        if (percentColorRules == null || percentColorRules.isBlank()) {
            percentColorRules = "percent<30=#FFFF2020;percent<50=#FFFFB000";
        }
    }

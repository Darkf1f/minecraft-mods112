...
public final class C2MEmod implements ClientModInitializer {
    ...
    /**
     * Draws the normal entity renderer with depth testing disabled. This is
     * intentionally separate from glow: the selected mob keeps its original
     * model and is visible through blocks.
     */
    private void renderThroughWalls(WorldRenderContext context) {
        ...
        BufferAllocator allocator = new BufferAllocator(16 * 1024);
        VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(allocator);
        VertexConsumer wallLines = immediate.getBuffer(RenderLayer.getLines());
        ...
        try {
            ...
            for (Entity entity : client.world.getEntities()) {
                ...
                drawEntityBox(wallLines, entity, cameraPos, mob.color(), 0.95f);
                 ...
                 try {
                     dispatcher.render(entity, entity.getX() - cameraPos.x, entity.getY() - cameraPos.y,
                             entity.getZ() - cameraPos.z, entity.getYaw(), context.matrixStack(),
                             immediate, 15728880);
                 } finally {
                     context.matrixStack().pop();
                 }
            }
            // The world event's shared consumers may already be closed at
            // this point. Flush our own provider while its builders are live.
            immediate.draw();
        } finally {
            allocator.close();
    ...
    private void drawMarkedBlocks(MinecraftClient client, ChunkMark mark, Vec3d cameraPos,
        ...
        float fillAlpha = clampOpacity(config.chunkOpacity);
        for (int x = mark.chunkX() * 16; x < mark.chunkX() * 16 + 16; x++) {
            for (int z = mark.chunkZ() * 16; z < mark.chunkZ() * 16 + 16; z++) {
                ...
                if (!isMarkedSurfaceBlock(block)) continue;
                ...
                // by the world pass, so every edge remains visible even when
                // the block is water or is partly hidden by nearby terrain.
                drawCube(lines, bx, by, bz,
                        r, g, b, borderAlpha);
                if (fills != null) {
                    drawTopFill(fills, bx + 0.001, by + 1.002, bz + 0.001,
                            r, g, b, fillAlpha * 0.35f);
                }
            }
        }
    }

    private static boolean isMarkedSurfaceBlock(BlockState state) {
        String id = Registries.BLOCK.getId(state.getBlock()).toString().toLowerCase(Locale.ROOT);
        return id.endsWith(":grass_block") || id.contains("dirt") || id.contains("podzol")
                || id.contains("mycelium") || id.contains("leaves") || id.contains("water");
    }
    ...
    private static void drawEntityBox(VertexConsumer v, Entity entity, Vec3d cameraPos,
        ...
        line(v, minX, minY, minZ, maxX, minY, minZ, r, g, b, alpha);
        line(v, maxX, minY, minZ, maxX, minY, maxZ, r, g, b, alpha);
        line(v, maxX, minY, maxZ, minX, minY, maxZ, r, g, b, alpha);
        line(v, minX, minY, maxZ, minX, minY, minZ, r, g, b, alpha);
        line(v, minX, maxY, minZ, maxX, maxY, minZ, r, g, b, alpha);
        line(v, maxX, maxY, minZ, maxX, maxY, maxZ, r, g, b, alpha);
        line(v, maxX, maxY, maxZ, minX, maxY, maxZ, r, g, b, alpha);
        line(v, minX, maxY, maxZ, minX, maxY, minZ, r, g, b, alpha);
        line(v, minX, minY, minZ, minX, maxY, minZ, r, g, b, alpha);
        line(v, maxX, minY, minZ, maxX, maxY, minZ, r, g, b, alpha);
        line(v, maxX, minY, maxZ, maxX, maxY, maxZ, r, g, b, alpha);
        line(v, minX, minY, maxZ, minX, maxY, maxZ, r, g, b, alpha);
    }

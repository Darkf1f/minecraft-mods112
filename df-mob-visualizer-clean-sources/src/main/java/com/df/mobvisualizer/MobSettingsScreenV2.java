...
public final class MobSettingsScreenV2 extends Screen {
    ...
    protected void init() {
        ...
        if (page.equals("main")) {
            ...
            button(left, 205, 310, "Очистка данных", b -> open("cleanup"));
            button(left, 240, 310, "Готово", b -> close());
            return;
        }
        title(pageTitle());
        ...
        for (int i = start; i < end; i++) {
            ...
            button(left, y, 310, name + suffix, b -> {
                if (page.equals("colors")) {
                    int old = colors.getOrDefault(id, defaultColor(id));
                    colors.put(id, nextColor(old));
                    config.customMobColors = serializeColors(colors);
                } else {
    ...
    private int nextColor(int color) {
        int[] colors = {0xFFFF4040, 0xFFFFAA00, 0xFFFFFF40, 0xFF40FF80, 0xFF40C0FF, 0xFFB060FF, 0xFFFF60C0};
        for (int i = 0; i < colors.length; i++) if (colors[i] == color) return colors[(i + 1) % colors.length];
        return colors[0];
    }
    private String hex(int color) { return String.format("#%08X", color); }
    private void save() { config.normalize(); config.save(); }
    @Override public void close() { save(); MinecraftClient.getInstance().setScreen(parent); }
}
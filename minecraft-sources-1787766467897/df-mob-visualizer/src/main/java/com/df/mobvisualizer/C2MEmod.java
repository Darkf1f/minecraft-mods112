...
public final class C2MEmod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // Both passes use their own BufferBuilder.  They are therefore safe
        // at LAST and cannot write to Fabric's shared world buffer after it
        // has been flushed (the old implementation caused "Not building!").
    ...
    private void tick(MinecraftClient client) {
        ...
        for (Entity entity : client.world.getEntities()) {
            ...
            // Chunk history is mob history. Players and non-living client
            // entities may still appear in the HUD, but never create a chunk
            // mark.
     ...
     * dedicated position/color buffer so the pass remains valid after the
     * normal world buffer has been flushed.
     */
    private void renderThroughWalls(WorldRenderContext context) {
        if (!config.enabled || !mobHighlightsOpen || !config.seeThroughMobs || state == null
                ...
                || context.matrixStack() == null) return;
        MinecraftClient client = MinecraftClient.getInstance();
        Vec3d cameraPos = context.camera().getPos();
        RenderSystem.setShader(ShaderProgramKeys.POSITION_COLOR);
        BufferBuilder wallLines = beginBuffer(VertexFormat.DrawMode.DEBUG_LINES);
    ...
    private void renderChunks(WorldRenderContext context) {
        // Saved chunk history must remain renderable after flying away or
        // restarting with scanning disabled. Scanning uses config.enabled;
        // drawing the persisted map only needs the overlay toggle.
        if (!chunksOpen || state == null) return;
        ...
        try {
            RenderSystem.setShader(ShaderProgramKeys.POSITION_COLOR);
            BufferBuilder vertices = beginBuffer(VertexFormat.DrawMode.DEBUG_LINES);
            BufferBuilder blockFills = config.showChunkFill
                    ? beginBuffer(VertexFormat.DrawMode.QUADS) : null;
            RenderSystem.lineWidth(2.0f);
            ...
            for (ChunkMark mark : state.visibleChunks()) {
                ...
                if (!client.world.isChunkLoaded(mark.chunkX(), mark.chunkZ())) continue;
            ...
            if (blockFills != null) flushBuffer(blockFills);
            flushBuffer(vertices);
        } catch (Exception e) {
            // Protect render pipeline against unexpected vertex/matrix crash
        } finally {
            RenderSystem.depthMask(true);
            RenderSystem.enableDepthTest();
    ...
    private void drawMarkedBlocks(MinecraftClient client, ChunkMark mark, Vec3d cameraPos,
                                  BufferBuilder lines, BufferBuilder fills,
        ...
        int[] heights = surfaceHeightCache.computeIfAbsent(
                MobOverlayState.chunkKey(mark.chunkX(), mark.chunkZ()),
                key -> surfaceHeights(client, mark.chunkX(), mark.chunkZ()));
        for (int x = mark.chunkX() * 16; x < mark.chunkX() * 16 + 16; x++) {
            for (int z = mark.chunkZ() * 16; z < mark.chunkZ() * 16 + 16; z++) {
                int localX = x - mark.chunkX() * 16;
                int localZ = z - mark.chunkZ() * 16;
                int y = heights[localX * 16 + localZ];
                if (y < client.world.getBottomY()) continue;

                double bx = x - cameraPos.x;
                double by = y - cameraPos.y;
                double bz = z - cameraPos.z;
                // Waypoint-style surface overlay: only the top face is
                // outlined, avoiding 12 edges per column and keeping the
                // persistent map cheap even with many marked chunks.
                drawTopOutline(lines, bx, by + 1.003, bz,
                        r, g, b, borderAlpha);
                if (fills != null) {
                    drawTopFill(fills, bx + 0.015, by + 1.015, bz + 0.015,
                            r, g, b, fillAlpha);
                }
            }
        }
        // Four short poles make the marked area visible while flying without
        // replacing blocks or adding entities to the client world.
        drawWaypointCorners(lines, mark, cameraPos, heights, r, g, b, borderAlpha);
    }
    ...
    private static int[] surfaceHeights(MinecraftClient client, int chunkX, int chunkZ) {
        int[] heights = new int[16 * 16];
        for (int localX = 0; localX < 16; localX++) {
            for (int localZ = 0; localZ < 16; localZ++) {
                int x = chunkX * 16 + localX;
                int z = chunkZ * 16 + localZ;
                // WORLD_SURFACE returns the first air position above the
                // visible terrain, so store the actual top block Y.
    ...
    private void drawWaypointCorners(BufferBuilder lines, ChunkMark mark, Vec3d cameraPos,
                                     int[] heights, float r, float g, float b, float alpha) {
        int[][] corners = {{0, 0}, {15, 0}, {15, 15}, {0, 15}};
        for (int[] corner : corners) {
            int x = mark.chunkX() * 16 + corner[0];
            int z = mark.chunkZ() * 16 + corner[1];
            int y = heights[corner[0] * 16 + corner[1]];
            double bx = x - cameraPos.x + (corner[0] == 15 ? 0.98 : 0.02);
            double bz = z - cameraPos.z + (corner[1] == 15 ? 0.98 : 0.02);
            linePlain(lines, bx, y - cameraPos.y + 1.02, bz,
                    bx, y - cameraPos.y + 4.0, bz, r, g, b, alpha);
        }
    }

    private static void drawCube(BufferBuilder v, double x, double y, double z,
    ...
    private static void drawTopFill(BufferBuilder v, double x, double y, double z,
                                    float r, float g, float b, float a) {
    ...
    private static void drawTopOutline(BufferBuilder v, double x, double y, double z,
                                       float r, float g, float b, float a) {
        double x2 = x + 0.998;
        double z2 = z + 0.998;
        line(v, x, y, z, x2, y, z, r, g, b, a);
    ...
    private static void drawEntityBox(BufferBuilder v, Entity entity, Vec3d cameraPos,
    ...
    private static void linePlain(BufferBuilder v, double x1, double y1, double z1,
    ...
    private static void drawFill(BufferBuilder v, double x, double y, double z,
                                 float r, float g, float b, float a) {
        double x2 = x + 16;
        double z2 = z + 16;
        v.vertex((float) x, (float) y, (float) z).color(r, g, b, a);
    ...
    private static void drawBox(BufferBuilder v, double x, double y, double z,
                                float r, float g, float b, float a) {
        double x2 = x + 16;
        double z2 = z + 16;
    ...
    private static void line(BufferBuilder v, double x1, double y1, double z1,
                             double x2, double y2, double z2,
                             float r, float g, float b, float a) {
    ...
    private static BufferBuilder beginBuffer(VertexFormat.DrawMode mode) {
        return Tessellator.getInstance().begin(mode, VertexFormats.POSITION_COLOR);
    }

    private static void flushBuffer(BufferBuilder builder) {
